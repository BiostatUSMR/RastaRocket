...
#'   randomization dates.
#' @param df_pat_soc_grade_date A data frame with patient IDs, SOC terms, AE grades, start 
...
#' @import dplyr ggplot2 forcats viridis glue
#' @export
#'
#' @examples
#' 
...
plot_patient_panchart <- function(df_soc_pt,
  ...
  df_plot <- df_patient |> 
    ...
    mutate(across(c("start", "end", "rando_date", "treatment_date"),
           ...
           pt = as.factor(pt),
           pt = forcats::fct_relevel(pt, "Treatment"))
  ...
  p <- df_plot |> 
    ggplot(mapping = aes(color = grade, xmin = start, xmax = end, y = soc)) +
    ...
    facet_grid(pt ~ ., scales = "free" , space = "free", switch = "y") +
    theme_bw() +
