...
#'
#' @export
df_builder_ae_v1 <- function(df_pat_grp,
                          df_pat_llt,
                          ref_grp = NULL){
  ...
  ### Modifs ########################################
  df_confidence_interval <- lapply(vec_pt, function(pt_i) {
    df_i <- df_freq_pat |> dplyr::filter(EIPTN == pt_i)


    nb_disease_exposed <- df_i |>
      dplyr::filter(RDGRPNAME == vec_grp[1]) |>
      dplyr::pull(nb_pat)

    nb_disease_nonexposed <- df_i |>
      dplyr::filter(RDGRPNAME == vec_grp[2]) |>
      dplyr::pull(nb_pat)

    pop_at_risk_exposed <- df_i |>
      dplyr::filter(RDGRPNAME == vec_grp[1]) |>
      dplyr::pull(nb_pat_per_grp)

    pop_at_risk_nonexposed <- df_i |>
      dplyr::filter(RDGRPNAME == vec_grp[2]) |>
      dplyr::pull(nb_pat_per_grp)

    #  Teste valeur manquante/ retourner une ligne NA
    if (length(nb_disease_exposed) != 1 ||
        length(nb_disease_nonexposed) != 1 ||
        length(pop_at_risk_exposed) != 1 ||
        length(pop_at_risk_nonexposed) != 1) {
    ...
    res <- riskdifference(
      a  = nb_disease_exposed,
      b  = nb_disease_nonexposed,
      N1 = pop_at_risk_exposed,
      N0 = pop_at_risk_nonexposed
    )
