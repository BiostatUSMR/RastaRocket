# tab_ei_tot_SOC <- function(data, data_2, soc_name,pt_name, var_group){
#   
#   
...
#   
#   
#   ## Ajout d'une modalité (total) en plus dans la variable soc_name pour trier 
#   
#   data_SOC_tot <- data %>% 
#     dplyr::mutate(soc_name= as.factor(soc_name))
#   
#   # Obtenez les niveaux actuels de la variable
#   niveaux_actuels <- levels(data_SOC_tot[[soc_name_sym]])
#   
#   # Ajouter "Total" aux niveaux actuels
#   nouveaux_niveaux <- c(niveaux_actuels, "Total")
#   
#   
...
#   data_tot <- data %>% 
#     dplyr::mutate(soc_name = as.factor(soc_name), 
#            soc_name  = factor(soc_name, levels = nouveaux_niveaux))
#   
#   
#   
#   ### Description du SOC (total)
#   
#   
#   tab_test  <-  data_tot %>%
#     dplyr::filter(!!soc_name_sym == "Total") %>% 
...
#       ei_percent_per = as.numeric(round(nb_ei / nb_ei_all_soc * 100, 2)),
#       nb_patient_per = as.numeric(round(nb_patient / n_tot_pat * 100, 2))
#     ) %>%
#     dplyr::relocate(ei_percent_per, .after = nb_ei) %>%
#     dplyr::relocate(nb_patient_per, .after = nb_patient) %>% 
#     dplyr::left_join(
#       data_tot %>% dplyr::filter(
#         !!var_group_sym == levels(data[[var_group]])[1]) %>%
...
#         dplyr::mutate(
#           pt_name = trimws(pt_name),
#           ei_percent = as.numeric(round(`Nombre d'EIs` / nb_ei_BRASA * 100, 2)),
#           nb_patient = as.numeric(round(`Nombre de patients` / nb_pat_BRASA * 100, 2))
#         ) %>%
...
#     gt::tab_stubhead(label = md("**Evénements indésirables**"))  %>% 
...
#     gt::tab_spanner(label = md("**EI**"), columns = c("N", "%")) %>%
#     gt::tab_spanner(label = md("**Patients**"), columns = c("  N", "  %")) %>%
#     gt::tab_spanner(label = md(" **EI** "), columns = c(" N ", " % ")) %>%
#     gt::tab_spanner(label = md(" **Patients** "), columns = c("N  ", "%  ")) %>%
#     gt::tab_spanner(label = md("**EI** "), columns = c("N ", "% ")) %>%
#     gt::tab_spanner(label = md("**Patients** "), columns = c(" N", " %")) %>%
#     gt::tab_spanner(label = md("**TOTAL**"), spanners = c("**EI**", "**Patients**")) %>%
#     gt::tab_spanner(
#       label = md(paste0("**",levels(data[[var_group]])[1],"**")),
#       spanners = c(" **EI** ", " **Patients** ")
#     ) %>%
#     gt::tab_spanner(
#       label = md(paste0("**",levels(data[[var_group]])[2],"**")),
#       spanners = c("**EI** ", "**Patients** ")) %>%
#     gt::tab_header(title = "", subtitle = md("**Evenements indésirables( PT) par SOC (Tout les grades confondus)**"))
#   
#   
