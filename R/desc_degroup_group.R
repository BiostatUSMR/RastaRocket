#' Description of grouped tables with data management
#'
#' This function generates a statistical summary for grouped data using the
#' `gtsummary` package. It provides customized statistics for both categorical
#' and continuous variables, allowing for adjustments in titles, rounding, and
#' other formatting options.
#'
#' @param data1 Dataframe. The input data to generate the table.
#' @param table_title Character. Title of the table (default: "").
#' @param quali Character vector. A vector containing the names of qualitative
#'   variables to describe (default: NULL).
#' @param quanti Character vector. A vector containing the names of quantitative
#'   variables to describe (default: NULL).
...
#'   `{mean} ({sd})`, `{median} ({p25}; {p75})`, and `{min} ; {max}`.
#' - Columns and rows are formatted and customized with options for titles,
#'   rounding, footnotes, and group titles.
#' - The `by` argument is used to create grouped tables based on a specified grouping variable.
#'
#' @examples
#' # Example usage:
#' data <- data.frame(
...
desc_degroup_group <- function(data1,
                     table_title = "",
                     ## Titre de la table
                     quali = NULL,
